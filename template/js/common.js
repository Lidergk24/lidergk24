$(".form-subscribe").submit(function(e) {
    ...
    var badSubscribe = '<p class="error_message">Ооопссс... Имейл уже подписан!</p>';
    ...
    if (!patternMail.test(mail)) {
        ...
        errors += '<p class="error_message">Недопустимые символы EMAIL</p>';
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.result == 'success') {
                    ...
                    $('.subscribe_name').css('border', 'solid 1px rgba(0, 0, 0, 0.15)');
    ...
    $.post("/cart/addAjax/" + id + '?x=' + random, { count: count }, function(data) {
        if (reload) { location.reload(); return; } else if (typeof(display_cart_info) == "function") {
            ...
            if (data) {
                ...
                $(".basket__box_sum").html(data.TotalPrice.toFixed(2) + ' ₽');
            }
        }
    }, 'json');
...
function cart_show_ajax(reload_if_empty = false) {
    $.post("/cart/addAjax/0", function(data) {
        if (typeof(display_cart_info) == "function") {
            ...
            $(".basket__val").html(data.Count);
            if (data.TotalPrice) $(".basket__box_sum").html(data.TotalPrice.toFixed(2));
